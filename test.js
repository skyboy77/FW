...
// 增加了 region 字段，主要用于辅助电影 (Movie) 的 watch_providers 查询
const PLATFORM_MAP = {
    // 国际巨头
...
function buildItem(item, isMovie, platformName) {
    ...
    return {
        ...
        mediaType: mediaType,
        title: title,
        subTitle: `${typeTag} ${platformName} | ⭐ ${score}`,
...
async function loadPlatformList(params) {
    ...
    if (platform !== "all") {
        if (isMovie) {
            // 注意：像湖南卫视、tvN等传统电视台通常不具有电影 provider 属性
    ...
    if (category === "hot") {
        ...
        queryParams["vote_count.gte"] = 30; // 全球范围防刷门槛稍提高到30人
    }
